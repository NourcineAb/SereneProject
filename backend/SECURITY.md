...
| M1 | Medium | Keyword-only crisis detection | High-recall floor only; `ALLOWED_TECHNIQUE` enum validates LLM technique tags (L4); **clinician review** still recommended for keyword list |
| M2 | Medium | No account deletion / export / retention (RGPD Art. 15/17/20) | `DELETE /auth/me` (cascade-deletes user + sessions + messages + mood logs), `GET /auth/me/export` (full JSON export); User→Session and User→MoodLog relationships configured with `cascade="all, delete-orphan"`; **Mobile RGPD UI** added to Profile screen |
...
| M4 | Medium | Email enumeration on register (409) | Register now returns 201 with a valid token even if email exists (no 409 leak) |
| L1 | Low | Dockerfile runs as root | Non-root `USER appuser` added to Dockerfile |
| L4 | Low | Prompt injection can neutralize the in-prompt crisis RULE | Crisis enforcement lives in code (`coach.py`), not just the prompt; technique tag validated against `ALLOWED_TECHNIQUES` enum |

## Remaining (recommended before launch)
| ID | Severity | Issue | Suggested fix |
|---|---|---|---|
| M1b | Medium | Crisis detection is still keyword-only (high recall, low precision) | Add a classifier or LLM-based triage layer; always surface resources on any risk signal; **clinician review** of the keyword list & `CRISIS_REPLY` |
| M5 | Medium | No email service integration | Password reset and email verification tokens are logged server-side only. Integrate an email provider (SendGrid, Resend, etc.) to actually deliver emails. |
| L3 | Low | No structured logging / observability | Add structured logging (structlog), error tracking (Sentry), and APM for production monitoring |
