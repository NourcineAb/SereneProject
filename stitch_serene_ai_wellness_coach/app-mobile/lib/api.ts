...
export type ExportData = {
  ...
  sessions: Array<{ id: number; title: string; created_at: string; messages: Array<Record<string, unknown>> }>;
  mood_logs: Array<{ id: number; score: number; label: string; note: string | null; created_at: string }>;
};
...
export type WeeklyReport = {
  ...
  most_used_technique: string | null;
  streak_days: number;
  mood_distribution: { calme: number; joyeux: number; neutre: number; anxieux: number; fatigue: number };
...
export const api = {
  ...
  socialLogin: (provider: string, token: string, email: string, name: string) =>
    request<Token>('/auth/social-login', { method: 'POST', body: JSON.stringify({ provider, token, email, name }) }),
  ...
  journalExport: () => request<Array<{ id: number; mood_score: number; content: string; technique: string | null; created_at: string }>>('/journal/export'),
  ...
  communityJoinChallenge: (id: number) =>
    request<UserChallengeData>(`/community/challenges/${id}/join`, { method: 'POST' }),
  communityMyChallenges: () => request<UserChallengeData[]>('/community/challenges/my'),
  communityChallengeProgress: (id: number) =>
    request<UserChallengeData>(`/community/challenges/${id}/progress`, { method: 'POST' }),
};
